...
$(document).ready(function () {
    ...
    $("#divConsentForm").load(htmPath, function () {
        var _param = ['ConsentForm', _rtnQuery.pat, 0, 0, _LocationId];
...
function onGetDataSuccess(data, context) {
    if (context == 'GetDetails') {
        ...
        var _FileName = 'PatientThumbCaptureConsent/' + $qc.Cookie("locationid") + '/' + _rtnQuery.pat+'.txt';
        fn_getMime({ FilePath: _FileName }, 'getmatchconcentthumb');
        ...
        fn_sendInstantCommunication(JSONPath, "PatientConsentForm");
    }
...
function whatsup() {
    var _filename = 'PatientConsentForm_' + _rtnQuery.pat + '_' + _rtnQuery.form + '_' + _rtnQuery.rid
...
function fn_GenerateExamPaperPDF(FileName) {
    ...
    _print += '<head>';
    _print += '<title>Patient Consent Form</title>';
...
function onGeneratePDF(data, context) {
    if (context == 'DownloadExamQuesPaper') {
        ...
        strSQL += ' values(' + _rtnQuery.rid + ',' + _rtnQuery.pat + ',' + _LocationId + ',' + _rtnQuery.form + ',`' + _url + '`,`PatientConsentForm`,getdate())';
