...
$(document).ready(function () {
    if (typeof localStorage.getItem('RptPage') == 'undefined' || localStorage.getItem('RptPage')==null) {
        $("#chkHideShow").prop('checked', true);
        $(".clsHeaderHide").show();
    } else {
        if (localStorage.getItem('RptPage') == '1') {
            $("#chkHideShow").prop('checked', true);
            $(".clsHeaderHide").show();
        } else {
    ...
    $("#chkHideShow").click(function () {
        ...
        localStorage.setItem('RptPage', flag);
    });
    ...
    var _objParams = ["PrintReceiptFormat", _rtnQuery.q, _LocationId, _rtnQuery.pat, "Receipt"];
...
function onGetDataSuccess(data, context) {
    if (context == 'GetDetails') {
        ...
        if (obj) {
            ...
            $("#divcustometreatment").hide();
            var tc = 0;

            if (typeof obj[0]["CustomeReceipt"] != 'undefined') {
                if (typeof obj[0]["CustomeReceipt"][0] != 'undefined') {
                    var _t = '';
                    for (var a = 0; a < obj[0]["CustomeReceipt"].length; a++) {
                        _t += '<tr>';
                        _t += '<td style="border-right: 1px solid #ccc; border-Bottom: 1px solid #ccc;padding:10px;">' + obj[0]["CustomeReceipt"][a]["TreatmentDate"] + '</td>';
                        _t += '<td style="border-right: 1px solid #ccc; border-Bottom: 1px solid #ccc;padding:10px;">' + obj[0]["CustomeReceipt"][a]["TreatmentName"] + '</td>';
                        _t += '<td style="border-right: 1px solid #ccc; border-Bottom: 1px solid #ccc;padding:10px;">' + obj[0]["CustomeReceipt"][a]["ToothNo"] + '</td>';
                        _t += '<td style="border-right: 1px solid #ccc; border-Bottom: 1px solid #ccc;padding:10px;">' + obj[0]["CustomeReceipt"][a]["TreatCharges"] + '</td>';
                        _t += '</tr>';
                        tc += parseFloat(obj[0]["CustomeReceipt"][a]["TreatCharges"]);
                    }

                    _t += '<tr>';
                    _t += '<td colSpan="3" style="text-align:right;border-right: 1px solid #ccc; border-Bottom: 1px solid #ccc;">Total Charges</td>';
                    _t += '<td style="border-right: 1px solid #ccc; border-Bottom: 1px solid #ccc;padding:10px;">' + tc + '</td>';
                    _t += '</tr>';

                    $("#tblCustomeTreat").html(_t);
                    $("#divcustometreatment").show();
                }
            }
            ...
            for (var i = 0; i < q.length; i++) {
                ...
                if (tc == 0) {
                    ...
                    $("#amount").html(q[i]["amount"]);
                    $("#spnPayMode").html(q[i]["paymode"]);

                    $("#amountinword").html(inWords(q[i]["amount"]));
                    $("#treatmentname").html(' treatement <strong>' + q[i]["TreatmentName"] + '</strong>');
                } else {
                    $("#amount").html(tc);
                    $("#amountinword").html(inWords(tc));
                    $("#treatmentname").html(' following treatements.');
                }
        ...
        fn_sendInstantCommunication(JSONPath, "PatientReceipt");
...
function whatsup() {
    var _filename = 'PatientReceipt_' + _rtnQuery.pat + '_' + _rtnQuery.q;
...
function fn_GenerateExamPaperPDF(FileName) {
    ...
    _print += '<head>';
    _print += '<title>Print Receipt</title>';
...
function onGeneratePDF(data, context) {
    if (context == 'DownloadExamQuesPaper') {
        ...
        strSQL += ' values(' + _rtnQuery.q + ',' + _rtnQuery.pat + ',' + _LocationId + ',0,`' + _url + '`,`PatientReceipt`,getdate())';
