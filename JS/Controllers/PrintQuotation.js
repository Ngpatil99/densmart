...
$(document).ready(function () {
    if (typeof localStorage.getItem('QuoPage') == 'undefined' || localStorage.getItem('QuoPage') == null) {
        $("#chkHideShow").prop('checked', true);
        $(".clsHeaderHide").show();
    } else {
        if (localStorage.getItem('QuoPage') == '1') {
            $("#chkHideShow").prop('checked', true);
            $(".clsHeaderHide").show();
        } else {
    ...
    $("#chkHideShow").click(function () {
        ...
        localStorage.setItem('QuoPage', flag);
    });
    ...
    var _objParams = ["PrintQuotationFormat", _rtnQuery.q, _LocationId, _rtnQuery.pat, ""];
...
function onGetDataSuccess(data, context) {
    if (context == 'GetDetails') {
        ...
        if (obj) {
            ...
            var str = '';
            var q = obj[0]["Quotation"];
            var _GroupId = 1; var total = 0;
            for (var i = 0; i < q.length; i++) {
                $("#qno").html(q[i]["RptNo"]);
                if (_GroupId != parseInt(q[i]["GroupID"])) {
                    s1 = '<tr><td  colSpan="5" style="text-align:right;font-weight: 600;font-size: 16px;padding-right: 34px;">Total: ' + total + '</td></tr>';
                    s1 += '<tr><td  colSpan="5" style="text-align:center;">OR</td></tr>';
                    _GroupId = parseInt(q[i]["GroupID"]);
                    total = 0;
                } else {
                    s1 = '';
                }
                str += s1 + '<tr>';
                str += '<td style="text-align:center">' + q[i]["TreatmentDesc"] + '</td>';
                str += '<td style="text-align:center">' + q[i]["ToothNo"] + '</td>';
                str += '<td style="text-align:center">' + q[i]["NoOfUnits"] + '</td>';
                str += '<td style="text-align:center">' + q[i]["PerUnitCharges"] + '</td>';
                str += '<td style="text-align:center">' + q[i]["Subtotal"] + '</td>';
                str += '</tr>';
                total += parseInt(q[i]["Subtotal"]);

            }

            str += '<tr><td  colSpan="5" style="text-align:right;font-weight: 600;font-size: 16px;padding-right: 34px;">Total: ' + total + '</td></tr>';

            $("#tblQuotation").html(str);
        ...
        fn_sendInstantCommunication(JSONPath, "PatientQuotation");

    }
}

function whatsup() {
    var _filename = 'PatientQuotation_' + _rtnQuery.pat + '_' + _rtnQuery.q;
...
function fn_GenerateExamPaperPDF(FileName) {
    ...
    _print += '<head>';
    _print += '<title>Quotation Receipt</title>';
...
function onGeneratePDF(data, context) {
    if (context == 'DownloadExamQuesPaper') {
        ...
        strSQL += ' values(' + _rtnQuery.q + ',' + _rtnQuery.pat + ',' + _LocationId + ',0,`' + _url + '`,`PrintQuotation`,getdate())';
